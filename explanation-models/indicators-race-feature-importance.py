...
#%%
heart_csv_path = 'C:/Users/Rawan Alamily/Downloads/McSCert Co-op/explainable-ai-heart/predictive-models/personal-indicators-model/data/heart-og.csv'
...
#%%
model = keras.models.load_model("C:/Users/Rawan Alamily/Downloads/McSCert Co-op/explainable-ai-heart/predictive-models/personal-indicators-model/saved-race-model")
#%%
...
def f(X):
    ...
    race = tf.convert_to_tensor(X[:,9])
    diabetic = tf.convert_to_tensor(X[:,10]) #str
    activity = tf.convert_to_tensor(X[:,11]) #str
    health = tf.convert_to_tensor(X[:,12]) #str
    sleep = tf.convert_to_tensor(X[:,13],dtype=np.int64) #int
    asthma = tf.convert_to_tensor(X[:,14]) #str
    kidney = tf.convert_to_tensor(X[:,15]) #str
    skinCancer = tf.convert_to_tensor(X[:,16]) #str
    X_dict = {'bmi': bmi, 'smoking': smoking,'alcoholDrinking': alcohol, 'stroke': stroke, 'physicalHealth': physical,
             'mentalHealth': mental, 'diffWalk': walk, 'sex': sex, 'ageGroup': age, 'race': race, 'diabetic': diabetic, 'physicalActivity': activity, 
